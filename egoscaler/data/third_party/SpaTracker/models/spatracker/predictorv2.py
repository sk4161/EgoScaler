...
from spatracker.models.core.spatracker.spatracker import get_points_on_a_grid
from spatracker.models.core.model_utils import smart_cat
from spatracker.models.build_spatracker import (
    build_spatracker,
)
from spatracker.models.core.model_utils import meshgrid2d, bilinear_sample2d, smart_cat
...
class SpaTrackerPredictor(torch.nn.Module):
    ...
    def _compute_sparse_tracks(
        ...
        if queries is not None:
            ...
            queries = torch.cat([queries_extra], dim=1)
        ...
        video_depth = F.interpolate(
            video_depth, tuple(self.interp_shape), mode="nearest"
        )
        ...
        for i in range(queries.shape[1]):
            depth_interp_i = bilinear_sample2d(
                video_depth[queries[:, i : i + 1, 0].long()],
                queries[:, i : i + 1, 1],
                queries[:, i : i + 1, 2],
            )
        ...
        t0 = time.time()
        tracks, __, visibilities = self.model(
            rgbds=rgbds, queries=queries, iters=6, wind_S=wind_length
        )
