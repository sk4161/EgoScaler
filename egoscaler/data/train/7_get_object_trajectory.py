...
from egoscaler.data.tools import (
    iou, get_points_colors, prepare_dataset, execute_global_registration,
    refine_registration, compute_rotation, minimum_3Dbox
)
...
def main(args):
    ...
    for idx, data in enumerate(tqdm(all_data, desc="Processing Data")):
        ...
        original_duration = np.round(np.arange(timestamp-camera_cfg.time_window, timestamp+camera_cfg.time_window, sampling_rate), 3)
        ...
        for i, _t in enumerate(duration):
            pil_img = Image.open(f'{args.save_dir}/images/{dataset_name}/{video_uid}/{file_name}/{_t}.jpg')
        ...
        object_masks, _, object_scores = gsam_model.predict(
            pil_image, 
            [manipulated_object], 
            threshold=data_cfg.mani_obj_det_threshold
        )            
        ...
        if os.path.exists(f'{args.save_dir}/hods/{dataset_name}/{video_uid}/{file_name}.pkl'):
            with open(f'{args.save_dir}/hods/{dataset_name}/{video_uid}/{file_name}.pkl', 'rb') as f:
        ...
        if len(hod_res['obj-bbox']):
            ...
            hod_obj_mask[hod_res['obj-bbox'][0][1]:hod_res['obj-bbox'][0][3], hod_res['obj-bbox'][0][0]:hod_res['obj-bbox'][0][2]] = 1
            ious = [iou(hod_obj_mask, obj_mask) for obj_mask in object_masks]
            target_obj_mask = object_masks[np.argmax(ious)]
        else:
            target_obj_mask = object_masks[np.argmax(object_scores)] # most confident object
        ...
        if args.visualize:
            vis = Visualizer(save_dir="./viz_data", pad_value=0, linewidth=3, show_first_frame=0, tracks_leave_trace=10, fps=10)
            vis.visualize(clip, pred_tracks[..., :2], pred_visibility, query_frame=args.query_frame)
        ############################################################################################################

        # trajectory projection
        ############################################################################################################
        if os.path.exists(f'{args.save_dir}/bboxes/{dataset_name}/{video_uid}/{file_name}.json'):
            with open(f'{args.save_dir}/bboxes/{dataset_name}/{video_uid}/{file_name}.json', 'r') as f:
                bboxes = json.load(f)
        else:
            continue
        ...
        validness = (0 <= xs) & (xs < width) & (0 <= ys) & (ys < height) # check out of frame exists
        valid_frames = (np.sum(validness, axis=1) >= np.sum(validness[0])/2) # if some of frames our of range beyound thresh, skip the instance
        ...
        regist_flag = True
        start = time.time()
        for i, (_t, coords, depth, rgbd) in enumerate(zip(duration, pred_tracks, depths, rgbd_seq)):
            xs, ys, zs = np.round(coords[:,0]).astype(int), np.round(coords[:,1]).astype(int), coords[:,2]
            ...
            if i == 0:
                ...
                if not _t in transform_matrices:
                    ...
                    source_keypoints, source_feature = prepare_dataset(source, voxel_size)
                ...
                homogeneous_coords = np.concatenate([object_coords, np.ones((object_coords.shape[0], 1))], axis=-1)
                projected_coords_homogeneous = (transform @ homogeneous_coords.T).T
                projected_coords = projected_coords_homogeneous[:, :3] / projected_coords_homogeneous[:, 3][:, np.newaxis]
        ...
        if args.visualize:
            ...
            traj_rotvec[:,0] = focal_len_x*traj_rotvec[:,0]/traj_rotvec[:,2] + principal_point
            traj_rotvec[:,1] = focal_len_y*traj_rotvec[:,1]/traj_rotvec[:,2] + principal_point
            ...
            pil_image.save(f'{args.save_dir}/obs_images/{dataset_name}/{video_uid}/{file_name}.jpg')
            np.save(f'{args.save_dir}/depths/{dataset_name}/{video_uid}/{file_name}', obs_depth)
