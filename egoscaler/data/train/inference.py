...
if __name__ == "__main__":
    ...
    if args.data_type == "RGBD":
        ...
        ).permute(0, 3, 1, 2)  # Convert to tensor and permute to (N, C, H, W)
    ...
    model.to("cuda")
    viser = Visualizer(
        save_dir=out_dir, grayscale=True, fps=10, pad_value=0, tracks_leave_trace=5
    )
        ...
        if scale < 1:
            ...
            if depth_tensor is not None:
                if isinstance(depth_tensor, torch.Tensor):
                    ...
                    depth_tensor = T.Resize((new_h, new_w))(
                        torch.from_numpy(depth_tensor)
                    )
